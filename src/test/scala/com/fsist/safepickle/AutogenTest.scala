...
object AutogenTest {
  ...
  object C10 {
    implicit object pickler extends Pickler[C10, PicklingBackend] {
      override def pickle(t: C10, writer: PicklingBackend#PickleWriter, emitObjectStart: Boolean): Unit = {
        writer.writeInt(t.i)
      }
      override def unpickle(reader: PicklingBackend#PickleReader, expectObjectStart: Boolean): C10 =
        C10(reader.int)
    }
  }
...
class AutogenTest extends FunSuite with WrapperTester {
  ...
  test("Autogenerated pickler uses explicit pickler defined locally") {
    implicit object pickler extends Pickler[C13, PicklingBackend] {
      override def pickle(t: C13, writer: PicklingBackend#PickleWriter, emitObjectStart: Boolean): Unit = {
        writer.writeString(t.s)
      }
      override def unpickle(reader: PicklingBackend#PickleReader, expectObjectStart: Boolean): C13 =
        C13(reader.string)
    }
