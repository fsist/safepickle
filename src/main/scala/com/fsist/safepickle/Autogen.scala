  ...
  private def doGenerateVersioned[T: c.WeakTypeTag, TOld <: OldVersion[_] : c.WeakTypeTag](implicit debug: Debug): Expr[Pickler[T]] = {
    ...
    val ret = q""" {
          ...
          new Pickler[$ttype] {
            ...
            override val schema: Schema = $newPicklerName.schema match {
              case Schema.SObject(members, desc) =>
                val version = Schema.SInt(readOnly = true, hidden = true, default = Some($currentVersion))
                Schema.SObject(Schema.Member("$$version", version) :: members, desc)

              case other => throw new IllegalArgumentException("Pickler[" + $clazzName + "].schema must be a Schema.SObject; other types are not supported for versioning")
            }
