      ...
      if (params.isEmpty) {
        ...
        def schemaMembers = q"${
          for (info <- paramInfos) yield {
            ...
            q"Schema.SObjectMember($name, $schema, $required)"
          }
        }"
  ...
  private def generatePicklerWithChildren[T](parentSym: ClassSymbol, children: Option[List[ClassSymbol]] = None)
        ...
        sub.typeHint match {
          case Some(hint) =>
            q"""{
                ...
                val newMember = SObjectMember("$$type", SStringConst(typeOf[String], $hint))
    ...
    val ret = q"""{
          ...
          new Pickler[$ttype] {
            ...
            override val schema: Schema = SOneOf(ttag.tpe, $schemas)
    ...
    val ret = q""" {
          ...
          new Pickler[$ttype] {
            ...
            override val schema: Schema = {
              ...
              target.copy(members = SObjectMember("$$version", SIntConst(typeOf[Int], $currentVersion)) :: target.members)
            }
