...
object JodaTimePicklers {
  implicit object InstantPickler extends Pickler[Instant, PicklingBackend] with ConvertPickler[Instant, Long, PicklingBackend] with PrimitivePicklers {
    override implicit def otherPickler: Pickler[Long, PicklingBackend] = LongPickler
    override def convertFrom(other: Long): Instant = new Instant(other)
    override def convertTo(t: Instant): Long = t.getMillis
  }

  implicit object DateTimePickler extends Pickler[DateTime, PicklingBackend] with ConvertPickler[DateTime, Long, PicklingBackend] with PrimitivePicklers {
    override implicit def otherPickler: Pickler[Long, PicklingBackend] = LongPickler
    override def convertFrom(other: Long): DateTime = new DateTime(other)
    override def convertTo(t: DateTime): Long = t.getMillis
  }

  implicit object DurationPickler extends Pickler[Duration, PicklingBackend] with ConvertPickler[Duration, Long, PicklingBackend] with PrimitivePicklers {
    override implicit def otherPickler: Pickler[Long, PicklingBackend] = LongPickler
  ...
  implicit object IntervalPickler extends Pickler[Interval, PicklingBackend] with ConvertPickler[Interval, (Long, Long), PicklingBackend] with PrimitivePicklers with CollectionPicklers {
